...
SELECT film_id, title, description, film.length, rental_rate FROM film WHERE title LIKE '%Harry% %Potter%'

-- No luck finding your movie? Maybe you made a mistake spelling the name. Write a query to get the film ID, title, description, length and the rental rate of all the movies starting with the two first letters of your favorite movie.
SELECT film_id, title, description, film.length, rental_rate FROM film WHERE title LIKE 'Ha%'
...
SELECT c.customer_id, (first_name , last_name) AS full_name, p.amount, p.payment_date FROM customer AS c
FULL JOIN payment AS p
ON c.customer_id = p.customer_id
ORDER BY p.staff_id
